...
export async function POST(_: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  if (!session) {
    logServer("warn", "wake session not found", { sessionId: id, userId: user.uid });
  ...
  if (!currentScene) {
    logServer("warn", "wake missing current scene", { sessionId: id });
  ...
  const frameKey = latestSegment?.lastFrameKey ?? null;
  const currentSceneResumePrompt = (currentScene as { resumePrompt?: string | null }).resumePrompt ?? null;
  const reconstructed =
    currentSceneResumePrompt && currentSceneResumePrompt.trim().length > 0
      ? { startPrompt: currentSceneResumePrompt, worldState }
  ...
  const updatedSession = await db.$transaction(async (tx) => {
    ...
    const refreshed = await tx.worldSession.findUniqueOrThrow({
      where: { id },
      include: {
        scenes: {
          orderBy: { index: "asc" },
    ...
    return { refreshed, segmentId: segment.id };
  });

  return NextResponse.json({
    session: toSessionDetail(updatedSession.refreshed),
    segmentId: updatedSession.segmentId,
    startPrompt: reconstructed.startPrompt,
    frameKey,
  });
}
